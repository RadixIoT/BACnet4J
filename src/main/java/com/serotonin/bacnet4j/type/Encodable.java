...
abstract public class Encodable {
    ...
    protected static void peekTagData(final ByteQueue queue, final TagData tagData) {
        ...
        if (tagData.length == 5) {
            ...
            if (tagData.length == 254)
                ...
                tagData.length = toLong(queue.peek(peekIndex++)) << 24 | toLong(queue.peek(peekIndex++)) << 16 | toLong(
                        queue.peek(peekIndex++)) << 8 | toLong(queue.peek(peekIndex++));
        }
    ...
    protected static <T extends Encodable> T readSequenceType(final ByteQueue queue, final Class<T> clazz,
        ...
        T result;
        try {
            result = clazz.getConstructor(new Class[] {ByteQueue.class, Integer.TYPE}).newInstance(new Object[] {queue,
                    contextId});
        } catch (final Exception e) {
            throw new BACnetException(e);
        }
     ...
     * @throws BACnetException
     */
    private static Encodable readUnknown(final ByteQueue queue, final int contextId) throws BACnetException {
