...
public class DateRange extends BaseType implements DateMatchable {
    ...
    private void validateInput() {
        if (startDate.getYear() != Date.UNSPECIFIED_YEAR && endDate.getYear() == Date.UNSPECIFIED_YEAR || startDate
                .getYear() == Date.UNSPECIFIED_YEAR && endDate.getYear() != Date.UNSPECIFIED_YEAR)
        ...
        if (endDate.getMonth() == Month.EVEN_MONTHS || endDate.getMonth() == Month.ODD_MONTHS)
            throw new BACnetRuntimeException("even/odd months are not supported in date ranges");
        if (startDate.getMonth() != Month.UNSPECIFIED && endDate.getMonth() == Month.UNSPECIFIED || startDate
                .getMonth() == Month.UNSPECIFIED && endDate.getMonth() != Month.UNSPECIFIED)
            throw new BACnetRuntimeException("start and end months must both be specific or unspecific");
        if (startDate.getDay() != Date.UNSPECIFIED_DAY && endDate.getDay() == Date.UNSPECIFIED_DAY || startDate
                .getDay() == Date.UNSPECIFIED_DAY && endDate.getDay() != Date.UNSPECIFIED_DAY)
            throw new BACnetRuntimeException("start and end day must both be specific or unspecific");
        if ((startDate.getDay() == Date.UNSPECIFIED_DAY && startDate
                .getDayOfWeek() != DayOfWeek.UNSPECIFIED) || (endDate.getDay() == Date.UNSPECIFIED_DAY && endDate
                        .getDayOfWeek() != DayOfWeek.UNSPECIFIED))
