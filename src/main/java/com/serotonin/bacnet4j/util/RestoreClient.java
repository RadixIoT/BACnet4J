...
public class RestoreClient {
     ...
     *
     * @param files
     *                                  the files to restore to the target device. The file names are expected to look
     *                                  like those produced
     ...
     * @throws IOException
     */
    public void begin(final List<File> files, final long restoreStateChangeTimeout) throws BACnetException,
            IOException {
    ...
    private void copyFiles(final List<File> files, final RemoteDevice rd) throws BACnetException, IOException {
        ...
        for (final File file : files) {
            ...
            if (fileOid == null) {
                ...
                final CreateObjectAck ack = localDevice.send(rd, new CreateObjectRequest(ObjectType.file,
                        new SequenceOf<>(new PropertyValue(PropertyIdentifier.objectName, new CharacterString(file
                                .getName())), new PropertyValue(PropertyIdentifier.fileType, new CharacterString(
                                        "configurationFile")), new PropertyValue(PropertyIdentifier.fileAccessMethod,
                                                FileAccessMethod.streamAccess)))).get();
            ...
            if (fileAccessMethod.equals(FileAccessMethod.recordAccess)) {
                ...
                try (FileInputStream in = new FileInputStream(file)) {
                    while (true) {
                        ...
                        reqCount++;
                        final AtomicWriteFileRequest req = new AtomicWriteFileRequest(fileOid, new StreamAccess(
                                new SignedInteger(currentPosition), data));
                        localDevice.send(rd, req).get();
