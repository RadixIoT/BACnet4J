...
public class PropertyUtils {
     ...
     * @param deviceTimeout
     *                      the timeout for the lookup of devices that are not currently known. A value <= 0 means to
     *                      use
     *                      the default timeout.
     * @return
     */
        ...
        try {
            ...
            for (final Map.Entry<ObjectIdentifier, List<PropertyReference>> obj : propRefs.getProperties().entrySet()) {
                for (final PropertyReference ref : obj.getValue()) {
                    updateResultAndCallback(result, callback, deviceId, obj.getKey(), ref.getPropertyIdentifier(), ref
                            .getPropertyArrayIndex(), error, completedProperties, totalProperties);
                }
            }
