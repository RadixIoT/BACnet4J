...
public class BackupClient {
     ...
     * @throws IOException
     */
    public List<File> begin(final File saveDir, final long backupStateChangeTimeout) throws BACnetException,
            IOException {
    ...
    private List<File> copyFiles(final File saveDir, final RemoteDevice rd) throws BACnetException, IOException {
        ...
        int octetsPerRequest = rd.getMaxAPDULengthAccepted() - ComplexACK.getHeaderSize(true) - AtomicReadFileAck
                .getHeaderSize() - AtomicReadFileAck.StreamAccessAck.getHeaderSize();
            ...
            try {
                if (fileAccessMethod.equals(FileAccessMethod.recordAccess)) {
                    ...
                    try (PrintWriter out = new PrintWriter(localFile)) {
                        while (true) {
                            reqCount++;
                            final AtomicReadFileRequest req = new AtomicReadFileRequest(fileOid, new RecordAccess(
                                    new SignedInteger(currentRecord), new UnsignedInteger(recordsPerRequest)));
                    ...
                    try (FileOutputStream out = new FileOutputStream(localFile)) {
                        while (true) {
                            reqCount++;
                            final AtomicReadFileRequest req = new AtomicReadFileRequest(fileOid, new StreamAccess(
                                    new SignedInteger(currentPosition), new UnsignedInteger(octetsPerRequest)));
                            final AtomicReadFileAck ack = localDevice.send(rd, req).get();
