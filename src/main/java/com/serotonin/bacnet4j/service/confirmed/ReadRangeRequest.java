...
public class ReadRangeRequest extends ConfirmedRequestService {
    ...
    public AcknowledgementService handle(final LocalDevice localDevice, final Address from) throws BACnetException {
        try {
            ...
            if (ack == null) {
                ...
                ack = new ReadRangeAck(objectIdentifier, propertyIdentifier, propertyArrayIndex, new ResultFlags(false,
                        false, false), UnsignedInteger.ZERO, new SequenceOf<>(), null);
            }
            return ack;
    ...
    private ReadRangeAck readRangeBySequenceNumber(final RangeReadable<?> list, final BySequenceNumber sequenceNumber)
        ...
        synchronized (list) {
            ...
            @SuppressWarnings("unchecked") final int pos = binarySearch((RangeReadable<Sequenced>) list,
                    new Sequenced() {
                        @Override
    ...
    private ReadRangeAck readRangeByTime(final RangeReadable<?> list, final ByTime time) throws BACnetServiceException {
        ...
        synchronized (list) {
            ...
            // Use a binary search to find the index of the record we need.
            @SuppressWarnings("unchecked") int pos = binarySearch((RangeReadable<Timestamped>) list, new Timestamped() {
                @Override
