...
public class ReadPropertyMultipleRequest extends ConfirmedRequestService {
    ...
    public AcknowledgementService handle(final LocalDevice localDevice, final Address from) throws BACnetException {
        ...
        for (final ReadAccessSpecification req : listOfReadAccessSpecs) {
            ...
            if (oid.getObjectType().equals(ObjectType.device) && oid
                    .getInstanceNumber() == ObjectIdentifier.UNINITIALIZED) {
    ...
    private static void addProperty(final BACnetObject obj, final List<Result> results, final PropertyIdentifier pid,
        ...
        if (obj == null) {
            ...
            for (final ObjectPropertyTypeDefinition def : ObjectProperties.getObjectPropertyTypeDefinitions(obj.getId()
                    .getObjectType())) {
            ...
            for (final ObjectPropertyTypeDefinition def : ObjectProperties.getRequiredObjectPropertyTypeDefinitions(obj
                    .getId().getObjectType())) {
                // Do not add the property list
                if (def.getPropertyTypeDefinition().getPropertyIdentifier() != PropertyIdentifier.propertyList) {
                    addNonSpecialProperty(obj, results, def.getPropertyTypeDefinition().getPropertyIdentifier(), pin,
            ...
            for (final ObjectPropertyTypeDefinition def : ObjectProperties.getOptionalObjectPropertyTypeDefinitions(obj
                    .getId().getObjectType())) {
    ...
    private static void addNonSpecialProperty(final BACnetObject obj, final List<Result> results,
        ...
        try {
            ...
            if (ignoreNotFound && e.getErrorClass() == ErrorClass.property && e
                    .getErrorCode() == ErrorCode.unknownProperty) {
                // ignore
            } else {
