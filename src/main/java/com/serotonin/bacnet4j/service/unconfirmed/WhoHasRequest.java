...
public class WhoHasRequest extends UnconfirmedRequestService {
    ...
    public void handle(final LocalDevice localDevice, final Address from) throws BACnetException {
        ...
        if (limits != null) {
            ...
            if (localId < limits.getDeviceInstanceRangeLowLimit().intValue() || localId > limits
                    .getDeviceInstanceRangeHighLimit().intValue())
                return;
        }
        ...
        if (result != null) {
            ...
            final IHaveRequest response = new IHaveRequest(localDevice.getId(), result.getId(), (CharacterString) result
