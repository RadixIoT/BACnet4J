                ...
                synchronized (e.getValue()) {
                    for (final CovContext ctx : e.getValue()) {
                        if (!ctx.hasExpired(now)) {
                            final RecipientProcess rp = new RecipientProcess(new Recipient(ctx.getAddress()), ctx
                                    .getSubscriberProcessIdentifier());
                            final ObjectPropertyReference opr = new ObjectPropertyReference(e.getKey(), ctx
                                    .getExposedMonitoredProperty(), null);
                            final CovSubscription cs = new CovSubscription(rp, opr, Boolean.valueOf(ctx
                                    .isIssueConfirmedNotifications()), new UnsignedInteger(ctx.getSecondsRemaining(
                                            now)), ctx.getCovIncrement());
