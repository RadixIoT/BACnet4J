...
public class AlgoReportingMixin extends EventReportingMixin {
    ...
    protected StateTransition evaluateEventState(final BACnetObject bo, final EventAlgorithm eventAlgo) {
        return eventAlgo.evaluateAlgorithmicEventState(bo, monitoredPropertyValue, objectPropertyReference
                .getObjectIdentifier(), additionalValues, eventParameter);
    }
    ...
    protected Reliability evaluateFaultState(final Encodable oldMonitoredValue, final Encodable newMonitoredValue,
        ...
        return faultAlgo.evaluateAlgorithmic(oldMonitoredValue, newMonitoredValue, bo.get(
                PropertyIdentifier.reliability), objectPropertyReference.getObjectIdentifier(), additionalValues,
                faultParameter);
    }
    ...
    protected PropertyValue getEventEnrollmentMonitoredProperty(final PropertyIdentifier pid) {
        ...
        if (pid.equals(objectPropertyReference.getPropertyIdentifier())) {
            return new PropertyValue(objectPropertyReference.getPropertyIdentifier(), objectPropertyReference
                    .getPropertyArrayIndex(), monitoredPropertyValue, null);
        }

        final Encodable value = additionalValues.get(new ObjectPropertyReference(objectPropertyReference
                .getObjectIdentifier(), pid));
        if (value == null) {
