 ...
 * ============================================================================
 *
 * Copyright (C) 2018 Radix IoT LLC. All rights reserved.
 *
...
public class RealtimeDriver {
    ...
    public void configure(String portId, int baud, byte thisStation, int maxMaster, int maxInfoFrames, int usageTimeout)
        ...
        //modprobe the driver
        ProcessBuilder pb = new ProcessBuilder("insmod", driver.getAbsolutePath());
        ...
        pb = new ProcessBuilder(configProgram.getAbsolutePath(), "-d" + portId, "-b" + baud, "-t" + thisStation,
                "-m" + maxMaster, "-f" + maxInfoFrames, "-u" + usageTimeout);
        pb.redirectError(Redirect.INHERIT);
        pb.redirectOutput(Redirect.INHERIT);
        process = pb.start();
        process.waitFor();
