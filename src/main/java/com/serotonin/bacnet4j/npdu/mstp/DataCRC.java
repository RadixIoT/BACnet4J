...
public class DataCRC {
    ...
    private static int calcDataCRC(int dataValue, int crcValue) {
        ...
        int crc =
                (crcValue >> 8) ^ (crcLow << 8) ^ (crcLow << 3) ^ (crcLow << 12) ^ (crcLow >> 4) ^ (crcLow & 0x0f) ^ ((crcLow & 0x0f) << 7);
        return crc & 0xffff;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Integer.toString(crc.getCrc(new Frame(FrameType.bacnetDataNotExpectingReply, (byte) 0xff,
                (byte) 8, new byte[] {0x1, 0x20, (byte) 0xff, (byte) 0xfc, (byte) 0xfe, 0x20, (byte) 0xa0, (byte) 0xe7,
                        (byte) 0x91, (byte) 0xf0, 0x3, 0x22, 0x1, 0x15, 0x2b, (byte) 0xf9, (byte) 0xff, 0x55,
                        (byte) 0xff, 0x0, 0x4})), 16));
        //        crc.accumulate(0xb8);
        //        crc.accumulate(0x96);
