...
public class IpNetwork extends Network implements Runnable {
     ...
     *                         if a timeout occurs, a NAK is received, the device is already registered as a foreign
     *                         device, or
    ...
    protected NPDU handleIncomingDataImpl(final ByteQueue queue, final OctetString linkService) throws Exception {
        ...
        if (function == 0x0) {
            ...
            throw new MessageValidationException("Unhandled BVLC function type: 0x" + Integer.toHexString(
                    function & 0xff));
