...
public class BackupRestoreTest {
    ...
    public void backupAndRestore() throws Exception {
        ...
        b.setReinitializeDeviceHandler(new DefaultReinitializeDeviceHandler() {
            ...
            @Override
            protected void createConfigurationFiles(final LocalDevice localDevice) throws BACnetServiceException,
                    IOException {
                final File streamFile = copy(file, "stream");
                final File recordFile = copy(file, "record");

                final FileObject stream = new FileObject(localDevice, localDevice.getNextInstanceObjectNumber(
                        ObjectType.file), "configurationFile", new StreamAccess(streamFile));
                final FileObject record = new FileObject(localDevice, localDevice.getNextInstanceObjectNumber(
                        ObjectType.file), "configurationFile", new CrlfDelimitedFileAccess(recordFile));
            ...
            protected void finishRestore(final LocalDevice localDevice) {
                try {
                    ...
                    final BACnetArray<ObjectIdentifier> configurationFiles = localDevice.getDeviceObject().get(
