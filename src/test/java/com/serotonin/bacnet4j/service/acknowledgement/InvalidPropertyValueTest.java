...
public class InvalidPropertyValueTest {
    ...
    public void testParse() {
        var deviceResults = new SequenceOf<Result>();
        ...
        deviceResults.add(new Result(PropertyIdentifier.maxSegmentsAccepted, propertyArrayIndex, errorResponse));

        // String protocol version which is Invalid as per the spec
        deviceResults.add(new Result(PropertyIdentifier.protocolVersion, new UnsignedInteger(0),
                new CharacterString(new CharacterEncoding(StandardCharacterEncodings.ANSI_X3_4), "hxzy-1.01")));

        var results = new SequenceOf<ReadAccessResult>();
        results.add(new ReadAccessResult(new ObjectIdentifier(ObjectType.device, 1003), deviceResults));
        ReadPropertyMultipleAck mockAck = new ReadPropertyMultipleAck(results);

        ByteQueue queue = new ByteQueue();
        mockAck.write(queue);

        BACnetErrorException e = assertThrows(BACnetErrorException.class, () -> new ReadPropertyMultipleAck(queue));
        TestUtils.assertErrorClassAndCode(e.getBacnetError().getError().getErrorClassAndCode(), ErrorClass.property,
