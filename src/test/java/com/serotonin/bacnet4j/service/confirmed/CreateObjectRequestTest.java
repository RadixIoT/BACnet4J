...
public class CreateObjectRequestTest extends AbstractTest {
    ...
    public void duplicateName() throws Exception {
        final CreateObjectError err = TestUtils.assertErrorAPDUException(() -> {
            d2.send(rd1, new CreateObjectRequest(ObjectType.group, new SequenceOf<>(new PropertyValue(
                    PropertyIdentifier.objectName, new CharacterString("existing group")), new PropertyValue(
                            PropertyIdentifier.listOfGroupMembers, new SequenceOf<>(new ReadAccessSpecification(
    ...
    public void badParameter() throws Exception {
        final CreateObjectError err = TestUtils.assertErrorAPDUException(() -> {
            d2.send(rd1, new CreateObjectRequest(ObjectType.group, new SequenceOf<>(new PropertyValue(
                    PropertyIdentifier.apduLength, UnsignedInteger.ZERO)))).get();
    ...
    public void createGroup() throws Exception {
        ...
        final CreateObjectAck ack = (CreateObjectAck) d2.send(rd1, new CreateObjectRequest(ObjectType.group,
                new SequenceOf<>(new PropertyValue(PropertyIdentifier.objectName, new CharacterString("new group")),
                        new PropertyValue(PropertyIdentifier.listOfGroupMembers, listOfGroupMembers)))).get();
    ...
    public void createNotificationClass() throws Exception {
        final CreateObjectAck ack = (CreateObjectAck) new CreateObjectRequest(ObjectType.notificationClass,
                new SequenceOf<>(new PropertyValue(PropertyIdentifier.objectIdentifier, new ObjectIdentifier(
                        ObjectType.notificationClass, 17)))).handle(d1, null);
