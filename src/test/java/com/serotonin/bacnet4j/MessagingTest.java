...
public class MessagingTest {
    ...
    public void readRequest() throws Exception {
        ...
        final LocalDevice d2 = new LocalDevice(2, new DefaultTransport(new TestNetwork(map, new Address(new byte[] {2}),
                200)));
        ...
        specs.add(new ReadAccessSpecification(new ObjectIdentifier(ObjectType.device, 2),
        ...
        @SuppressWarnings("unchecked") final SequenceOf<ObjectIdentifier> idList = (SequenceOf<ObjectIdentifier>) result
    ...
    public void segmentedRequest() throws Exception {
        ...
        for (int i = 0; i < 1000; i++)
            specs.add(new WriteAccessSpecification(new ObjectIdentifier(ObjectType.analogValue, i), new SequenceOf<>(
                    propertyValues)));
        ...
        final ReadPropertyAck ack = d1.send(r2, new ReadPropertyRequest(new ObjectIdentifier(ObjectType.analogValue,
                567), PropertyIdentifier.units)).get();
