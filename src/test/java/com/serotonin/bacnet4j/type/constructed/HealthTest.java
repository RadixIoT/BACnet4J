...
public class HealthTest {
    ...
    public void allFields() throws BACnetException {
        final Health h = new Health(
                new DateTime(new Date(2026, Month.JUNE, 12, DayOfWeek.FRIDAY), new Time(8, 30, 0, 0)),
                new ErrorClassAndCode(ErrorClass.object, ErrorCode.referencedPortInError),
                PropertyIdentifier.bbmdBroadcastDistributionTable,
                new CharacterString("BBMD unreachable"));

        final ByteQueue queue = new ByteQueue();
        h.write(queue);

        assertEquals(h, new Health(queue));
    }
